...
Apply Supabase migrations **`028`**–**`030`**, **`032`**, **`033`**, **`036`**, **`037`**, **`038`**, and **`039`** (`urgent_reminder_sent`, `urgent_reminder_last_sent_on`, `due_today_reminder_sent_on`, `creator_due_today_reminder_sent_on`, `creator_urgent_reminder_last_sent_on`; **`036`** adds `subtask.urgent_reminder_sent` and `subtask.creator_urgent_reminder_last_sent_on`; **`037`** adds `subtask.subtask_creator_due_today_reminder_sent_on`; **`038`** adds `subtask.assignee_urgent_reminder_last_sent_on`; **`039`** adds `subtask.subtask_assignee_due_today_reminder_sent_on`). **Urgent (80%)** emails run only on HK calendar days **before** the due date — assignees get “You have upcoming tasks due”; sub-task **assignees** get **“You have upcoming sub-tasks due”**; task **creators** get **“An upcoming task due”** to `task.create_by`; sub-task **creators** get **“An upcoming sub-task due”** to `subtask.create_by` (skipped when `create_by` equals `assignee_01`; not sent when HK today is the sub-task due date). **Due-today** emails when HK **today** equals `due_date`: **“You have sub-tasks due today”** to sub-task assignees; **“A sub-task due today”** to `subtask.create_by`. **`POST /api/cron/urgent-task-reminders`** returns JSON `{ ok, urgent, assigneeUrgentSubtask, creatorUrgent, creatorUrgentSubtask, dueToday, assigneeDueTodaySubtask, creatorDueToday, creatorDueTodaySubtask }`. **`POST /api/cron/due-today-reminders`** returns `{ ok, dueToday, assigneeDueTodaySubtask, creatorDueToday, creatorDueTodaySubtask }`. After the due date (HK), past-due cleanup clears the reminder columns on tasks and subtasks.
