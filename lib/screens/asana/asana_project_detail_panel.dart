...
class _AsanaProjectDetailPanelState extends State<AsanaProjectDetailPanel> {
  ...
  Future<void> _loadProject() async {
    ...
    final p = await DatabaseService.fetchProjectById(widget.projectId);
    if (!mounted) return;
    if (p != null) {
  ...
  Future<void> _loadComments() async {
    if (!PostgrestConfig.isConfigured) return;
    try {
      final list = await DatabaseService.fetchProjectComments(widget.projectId);
      if (!mounted) return;
  ...
  Future<void> _syncAssigneeKeysFromProject(ProjectRecord p) async {
    ...
    for (var i = 0; i < p.assigneeStaffUuids.length; i++) {
      ...
      final key = await DatabaseService.assigneeListKeyFromStaffUuid(u);
      keys.add(key);
  ...
  List<Map<String, String>> _projectChangesForEmail(
    ...
    _addChange(
      ...
      _namesFor(state, effectiveAssigneeIds),
    );
    _addChange(
      changes,
      'pic',
  ...
  Future<String?> _commitPendingInlineImages({
    ...
    for (final row in List<InlineAttachmentRow>.from(
      ...
      if (row.entityType == 'project_comment') {
        final touchErr = await DatabaseService.touchProjectCommentRow(
  ...
  Future<void> _savePostedCommentOnBlur(
    ...
    final err = await DatabaseService.updateProjectCommentRow(
  ...
  Future<String?> _postDraftCommentWithoutOverlay(AppState state) async {
    ...
    final c = await DatabaseService.insertProjectCommentRow(
      projectId: widget.projectId,
  ...
  Future<void> _deletePostedComment(ProjectCommentRowDisplay comment) async {
    ...
    _setSaving(true);
    try {
      final err = await DatabaseService.softDeleteProjectCommentRow(
        commentId: comment.id,
        updaterStaffLookupKey: state.userStaffAppId,
      );
      if (err != null && mounted) {
